...
const recipe = createMockInstagramRecipePost({
  recipe_data: createMockRecipeData({
    steps: [
      ...
      { idx: 2, text: "Stir-fry until crisp.", used_ingredients: [] },
    ],
    ingredients: [{ id: "1", name: "Beef", quantity: 200, unit: "g" }],
  }),
});
...
describe("StepsPanel", () => {
  it("renders steps and toggles active state", () => {
    ...
    expect(
      screen.getByRole("button", { name: /Cook mode/i })
    ).toBeInTheDocument();
  });
});
