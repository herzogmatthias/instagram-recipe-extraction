...
import { createMockInstagramRecipePost, createMockRecipeWithStatus, createMockRecipeWithMissingData } from '@/lib/utils/mockDataFactory';
...
describe('RecipeCard', () => {
  ...
  describe('Action buttons', () => {
    it('should show action buttons', () => {
      ...
      expect(screen.getByLabelText('Export recipe as JSON')).toBeInTheDocument();
      expect(screen.getByLabelText('More actions')).toBeInTheDocument();
    });

    it('should disable open button when status is not ready', () => {
  ...
  describe('Visual states', () => {
    ...
    it('should show progress bar for processing states', () => {
      const recipe = createMockInstagramRecipePost({
      ...
      // Should show for uploading_media status
      const progress = screen.queryByRole('progressbar');
      expect(progress).toBeInTheDocument();
    });

    it('should show progress bar for queued state', () => {
      const recipe = createMockInstagramRecipePost({
        displayUrl: undefined,
        recipe_data: undefined,
      });
    ...
    it('should not show progress bar for ready state', () => {
      ...
      // Should not show for ready status
      const progress = screen.queryByRole('progressbar');
      expect(progress).not.toBeInTheDocument();
    });

    it('should not show progress bar for extracting state', () => {
      const recipe = createMockInstagramRecipePost({
        recipe_data: {
          title: 'Test',
          ingredients: [],
      ...
      // Extracting should show progress
      const progress = screen.queryByRole('progressbar');
      expect(progress).toBeInTheDocument();
    });
  });
  ...
  describe('Accessibility', () => {
    ...
    it('should handle Enter key press', () => {
      ...
      const recipe = createMockInstagramRecipePost();
      const { container } = render(<RecipeCard recipe={recipe} onClick={onClick} />);
